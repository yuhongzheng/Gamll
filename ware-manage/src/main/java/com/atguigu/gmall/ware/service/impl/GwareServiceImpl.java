...
public class GwareServiceImpl implements GwareService {
     ...
     */
    @Override
    public Boolean closeWare(Long orderId) {
        if (orderId == null) {
            return false;
        }
        ...
        //查询运送状态
        Integer outCount = wareOrderTaskMapper.selectCount(new QueryWrapper<WareOrderTask>()
                .eq("order_id", orderId)
                .in("task_status", Arrays.asList(TaskStatus.DELEVERED.name(), TaskStatus.PAID.name())));
        if (outCount > 0) {
            return false; //有已发货商品或者未处理完毕
        }

        List<WareOrderTask> wareOrderTaskList = wareOrderTaskMapper.selectList(new QueryWrapper<WareOrderTask>()
                .eq("order_id", orderId)
                .in("task_status", Arrays.asList(TaskStatus.DEDUCTED.name(),
                        TaskStatus.OUT_OF_STOCK.name()), TaskStatus.SPLIT.name()
        ));
        //改变库存状态
        for (WareOrderTask wareOrderTask : wareOrderTaskList) {
            if (wareOrderTask.getTaskStatus().equals(TaskStatus.DEDUCTED.name())) {
                //改变库存数量
                List<WareOrderTaskDetail> details = wareOrderTaskDetailMapper.selectList(
                        new QueryWrapper<WareOrderTaskDetail>().eq("task_id", wareOrderTask.getId()));
                for (WareOrderTaskDetail detail : details) {
                    WareSku wareSku = wareSkuMapper.selectOne(new QueryWrapper<WareSku>().eq("sku_id", detail.getSkuId()));
                    wareSku.setStockLocked(wareSku.getStockLocked() - detail.getSkuNum());
                    wareSkuMapper.updateById(wareSku);
                }
            }

            wareOrderTask.setTaskStatus(TaskStatus.CLOSE.name());
    ...
    public Map<String, List<String>> getWareSkuMap(List<String> skuIdlist) {
        ...
        return wareSkuMap;
//        return wareSkuList.stream().collect(
//                Collectors.groupingBy(WareSku::getWarehouseId, //以仓库ID为key
//                Collectors.mapping(WareSku::getSkuId, Collectors.toList()))); //以skuId List为值

    }

    /**
     * 将wareSkuMap转换成列表
     * @param wareSkuMap map(仓库ID, skuIdList)
     * @return [map(wareId: ?, skuIds: list)]
     */
